 ...
 * 
 * Pipeline:
 * 1. BriefGenerator → product-brief.md (AI-powered)
 * 2. InspirationEngine → 0.design-moodboard.md
 * 3. SpecGenerator → STYLE_GUIDE.md + section specs
 * 4. SiteGenerator → Next.js components
 * 5. DeployService → Vercel deployment
 */
...
async function generateBrief(input, outputDir) {
    ...
    if (input.briefContent) {
        console.log('📄 Using pre-generated brief from n8n...');
        fs.writeFileSync(briefPath, input.briefContent);
        console.log(`✅ Saved: product-brief.md (from n8n)`);
        return briefPath;
    }
...
`;
        ...
        console.log(`✅ Generated: product-brief.md (AI-structured)`);
        return briefPath;

    } catch (error) {
        console.error('⚠️ AI brief generation failed, using template fallback:', error.message);
...
`;
    ...
    console.log(`✅ Generated: product-brief.md (template fallback)`);
    return briefPath;
}
...
async function runPipeline(job) {
    ...
    try {
        ...
        console.log('\n🏗️ Step 4/5: Generating Next.js Site...');
...
if (require.main === module) {
    const testJob = {
        ...
        input: {
            ...
            vibe: 'Minimalist Dark Portfolio',
            tagline: 'Building digital experiences',
            about: 'I create elegant solutions to complex problems.',
            skills: ['React', 'Node.js', 'TypeScript', 'UI/UX'],
            projects: [
                { name: 'Project Alpha', description: 'A revolutionary app' },
                { name: 'Project Beta', description: 'An innovative platform' }
            ],
            contact: { email: 'alex@example.com', github: 'alexchen' }
        }
    };
